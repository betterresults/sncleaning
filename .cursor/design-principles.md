...
| Primitive | Use for |
|-----------|---------|
| `ShellPage` | Every in-shell page wrapper; set `width` (`default` \| `narrow` \| `wide` \| `full`); use `fill` for master/detail tool pages |
...
| SMS feature | `src/features/sms/*` | **Tailwind** (shell workspace redesign) |
