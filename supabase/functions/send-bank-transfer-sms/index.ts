...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { bookingId, phoneNumber, customerName, amount, bookingDate }: SendBankTransferSMSRequest = await req.json();

    console.log(`Sending bank transfer SMS for booking ${bookingId} to ${phoneNumber}`);
    ...
    return new Response(
      JSON.stringify({ 
        ...
        message: 'Bank transfer SMS sent successfully'
      }),
      {
        status: 200,
