...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Check if account already exists for this cleaner
    ...
    console.log('Checking if user already exists with email:', cleaner.email);
    ...
    const existingUser = existingUsers.users.find(user => user.email === cleaner.email);

    if (existingUser) {
      console.log('Found existing user with email, linking to cleaner:', existingUser.id);
      authUser = { user: existingUser };
      
      // Update the user's metadata to include cleaner info
    ...
    // Create or update profile with cleaner_id
    ...
    if (profile) {
      ...
      if (profileError) {
        console.error('Error creating profile for cleaner:', profileError);
      }
    }

    // Send welcome email if requested and user was created (not linked)
    if (send_email && userCreated && tempPassword) {
              ...
              <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f8f9fa;">
                ...
                <div style="padding: 40px 30px; background-color: white;">
                  ...
                  <div style="text-align: center; margin: 35px 0;">
                    <a href="${supabaseUrl?.replace('.supabase.co', '.lovableproject.com') || 'https://your-domain.com'}/auth" 
    ...
    return new Response(JSON.stringify({
      ...
      user_id: authUser.user.id,
      email: cleaner.email,
      temp_password: userCreated && send_email ? tempPassword : null,
      action: userCreated ? 'created' : 'linked'
    }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    });
