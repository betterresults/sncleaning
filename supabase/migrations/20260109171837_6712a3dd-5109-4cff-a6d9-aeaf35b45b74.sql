-- Fix generate_recurring_bookings function to use correct column names
...
BEGIN
    ...
    -- Use correct column names: postponed (not status), confirmed
    FOR rs IN 
        SELECT * 
        FROM public.recurring_services
        WHERE postponed IS DISTINCT FROM TRUE
        AND confirmed = TRUE  -- Only process confirmed services
        AND start_date IS NOT NULL
        AND start_time IS NOT NULL  -- Skip services without a confirmed time
    ...
    LOOP
        ...
        WHERE customer = rs.customer  -- Use 'customer' not 'customer_id'
        AND date_time::date = rs.start_date
        AND (recurring_group_id IS NULL OR recurring_group_id::text != rs.recurring_group_id::text);
        
        IF existing_booking_id IS NOT NULL THEN
            UPDATE public.bookings 
            SET recurring_group_id = rs.recurring_group_id::text
        ...
        WHILE booking_date <= CURRENT_DATE + INTERVAL '56 days' AND loop_counter < max_iterations LOOP
            ...
            IF day_name = ANY(days_array) THEN
                IF NOT EXISTS (
                    ...
                    AND recurring_group_id::text = rs.recurring_group_id::text
                ) THEN
                    
                    booking_datetime := booking_date + rs.start_time;

                    -- Get address using 'address' column (which is uuid)
                    SELECT address, postcode INTO addr
                    FROM public.addresses 
                    WHERE id = rs.address;  -- Use 'address' not 'address_id'
                    ...
                    IF addr.address IS NOT NULL THEN
                        ...
                        VALUES (
                            rs.customer,  -- Use 'customer' not 'customer_id'
                            rs.cleaning_type, 
                            booking_datetime, 
                            addr.address, 
                            addr.postcode,
                            rs.cleaner,  -- Use 'cleaner' not 'cleaner_id'
                            rs.hours::numeric, 
                            rs.hours::numeric, 
                            rs.cost_per_hour, 
                            rs.total_cost,
