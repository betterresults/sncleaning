-- Update the generate_recurring_bookings function to handle postponed services and auto-resume
...
BEGIN
    ...
    FOR rs IN 
        ...
        WHERE postponed IS DISTINCT FROM TRUE  -- Only active services
            ...
            IF day_name = LOWER(TRIM(rs.days_of_the_week)) THEN
                ...
                IF NOT EXISTS (
                    ...
                    IF addr.address IS NOT NULL THEN
                        -- Create the booking with send_notification_email set to false for automatic bookings
