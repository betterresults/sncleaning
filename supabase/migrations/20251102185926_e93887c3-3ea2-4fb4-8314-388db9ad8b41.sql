-- Fix the generate_recurring_bookings function to respect the interval for bi-weekly and monthly services
-- The bug: it was incrementing by 1 day instead of jumping by the interval after creating a booking
...
BEGIN
    ...
    LOOP
        ...
        WHILE booking_date <= CURRENT_DATE + INTERVAL '30 days' AND loop_counter < max_iterations LOOP
            ...
            IF day_name = LOWER(TRIM(rs.days_of_the_week)) THEN
                IF NOT EXISTS (
                    ...
                    IF addr.address IS NOT NULL THEN
                        ...
                        RAISE NOTICE 'CREATED booking for % (Service % - Interval: % days)', booking_date, rs.id, rs.interval;
                    END IF;
                ELSE
            ...
            -- FIX: If a booking was created, jump by the interval; otherwise just move to the next day to find the matching day
            IF booking_created_this_iteration THEN
