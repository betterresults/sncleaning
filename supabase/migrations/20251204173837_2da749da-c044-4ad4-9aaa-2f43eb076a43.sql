-- Fix the send_booking_notification function to NOT use sms_template_id for notification_logs
-- since template_id is a foreign key to email_notification_templates only
...
BEGIN
  ...
  -- Prepare notification variables with booking_id included
  ...
  LOOP
    ...
    IF (trigger_record.notification_channel = 'sms' OR trigger_record.notification_channel = 'both') 
      ...
      sms_content := REPLACE(sms_content, '{{booking_date}}', TO_CHAR(booking_record.service_date, 'Day, DD Month YYYY'));
      ...
      -- Log the SMS notification - use NULL for template_id since it's a foreign key to email_notification_templates
      INSERT INTO notification_logs (
        trigger_id,
        template_id,
        recipient_email,
        ...
        trigger_record.id,
        NULL,  -- Cannot use sms_template_id here as template_id references email_notification_templates
        recipient_phone,
        recipient_type,
