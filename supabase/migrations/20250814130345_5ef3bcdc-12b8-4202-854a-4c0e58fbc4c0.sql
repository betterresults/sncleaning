-- Improved generate_recurring_bookings function with better error handling and performance
...
BEGIN
    ...
    RAISE NOTICE 'Starting generate_recurring_bookings function';
    ...
    FOR rs IN 
        ...
        LIMIT 10 -- Process max 10 services at a time to avoid timeout
    LOOP
        RAISE NOTICE 'Processing recurring service ID: %', rs.id;
        ...
        WHILE booking_date <= CURRENT_DATE + INTERVAL '30 days' AND loop_counter < max_iterations LOOP
            ...
            -- Move forward by interval (ensure it's a valid interval)
            BEGIN
                booking_date := booking_date + (rs.interval || ' days')::interval;
            EXCEPTION 
                WHEN OTHERS THEN
                    RAISE NOTICE 'Invalid interval for service ID %, skipping: %', rs.id, rs.interval;
                    EXIT; -- Exit the while loop for this service
            END;
        END LOOP;
