...
DO $$
DECLARE
    booking_confirmation_template_id uuid;
    booking_status_template_id uuid;
    booking_completion_template_id uuid;
    payment_reminder_template_id uuid;
BEGIN
    -- Get template IDs
    SELECT id INTO booking_confirmation_template_id FROM email_notification_templates WHERE name = 'booking_confirmation';
    SELECT id INTO booking_status_template_id FROM email_notification_templates WHERE name = 'booking_status_update';
    SELECT id INTO booking_completion_template_id FROM email_notification_templates WHERE name = 'booking_completion';
    SELECT id INTO payment_reminder_template_id FROM email_notification_templates WHERE name = 'payment_reminder';

    -- Insert notification triggers
    INSERT INTO notification_triggers (name, trigger_event, template_id, recipient_types, is_enabled) VALUES
    ('Booking Confirmation Email', 'booking_created', booking_confirmation_template_id, ARRAY['customer'], true),
    ('Booking Status Update Email', 'booking_status_changed', booking_status_template_id, ARRAY['customer'], true),
    ('Booking Completion Email', 'booking_completed', booking_completion_template_id, ARRAY['customer'], true),
    ('Payment Reminder Email', 'payment_overdue', payment_reminder_template_id, ARRAY['customer'], true)
    ON CONFLICT (name) DO NOTHING;
END $$;