...
interface CarpetExitQuotePopupProps {
  open: boolean;
...
const formatItemsForDisplay = (items: CarpetCleaningItem[]): string[] => {
...
export const CarpetExitQuotePopup: React.FC<CarpetExitQuotePopupProps> = ({
  open,
  onOpenChange,
  email: initialEmail,
  data,
  ...
  const handleSendQuote = async () => {
    ...
    setIsSending(true);
    try {
      const { data: responseData, error } = await supabase.functions.invoke('send-carpet-quote-email', {
        body: {
          email,
          quoteData: {
            carpetItems: data.carpetItems,
            upholsteryItems: data.upholsteryItems,
            mattressItems: data.mattressItems,
            totalCost: data.totalCost,
            shortNoticeCharge: data.shortNoticeCharge || 0,
  ...
  // Collect all items for summary
  const allCarpetItems = formatItemsForDisplay(data.carpetItems);
  const allUpholsteryItems = formatItemsForDisplay(data.upholsteryItems);
  const allMattressItems = formatItemsForDisplay(data.mattressItems);
  const hasItems = allCarpetItems.length > 0 || allUpholsteryItems.length > 0 || allMattressItems.length > 0;
              ...
              {hasItems && data.totalCost > 0 && (
                <div className="bg-slate-50 rounded-xl p-4 space-y-3 border border-slate-100">
                  ...
                  <div className="flex justify-between text-sm pt-2 border-t border-slate-200">
                    ...
                    <span className="font-semibold text-slate-800">£{data.totalCost.toFixed(2)}</span>
                  </div>
                </div>
              )}
