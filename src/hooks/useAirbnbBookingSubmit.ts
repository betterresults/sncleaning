...
interface BookingSubmission {
  ...
  // Recurring service costs (from quote)
  weeklyCost?: number; // Discounted weekly cost from quote (already includes first-time discount if applicable)
  regularHours?: number; // Regular weekly hours for recurring (may differ from first deep clean hours)
  isFirstTimeCustomer?: boolean; // Whether first-time 10% discount applies
...
export const useAirbnbBookingSubmit = () => {
  ...
  const submitBooking = async (bookingData: BookingSubmission, skipPaymentAuth = false) => {
    try {
      ...
      // Create recurring service for domestic/commercial recurring bookings
      const isRecurring = bookingData.serviceFrequency && 
        bookingData.serviceFrequency !== 'onetime' && 
        bookingData.serviceFrequency !== '';
      
      if (isRecurring && (bookingData.subServiceType === 'domestic' || bookingData.subServiceType === 'commercial')) {
        console.log('[useAirbnbBookingSubmit] Creating recurring service for frequency:', bookingData.serviceFrequency);
        
        try {
          const recurringGroupId = crypto.randomUUID();
          
          // Update the booking with the recurring group ID
          await supabase
            .from('bookings')
          ...
          // Calculate recurring cost - use quote weeklyCost if provided, otherwise calculate
          // The weeklyCost from quote already includes the 10% first-time discount if applicable
          const regularHours = bookingData.regularHours || bookingData.estimatedHours || bookingData.totalHours || 0;
          let recurringCost: number;
          let costPerHour: number;
          
          if (bookingData.weeklyCost && bookingData.weeklyCost > 0) {
            // Use the exact weekly cost from the quote (already has discount applied)
            recurringCost = bookingData.weeklyCost;
            costPerHour = regularHours > 0 ? recurringCost / regularHours : bookingData.hourlyRate;
            console.log('[useAirbnbBookingSubmit] Using quote weeklyCost:', recurringCost, 'costPerHour:', costPerHour);
          } else {
            // Calculate from hourly rate, applying first-time discount if applicable
            costPerHour = bookingData.hourlyRate;
            recurringCost = regularHours * costPerHour;
            if (bookingData.isFirstTimeCustomer) {
              recurringCost = recurringCost * 0.9; // 10% discount
              costPerHour = costPerHour * 0.9;
            }
            console.log('[useAirbnbBookingSubmit] Calculated recurring cost:', recurringCost, 'with discount:', bookingData.isFirstTimeCustomer);
          }
          ...
          const recurringServiceData = {
            ...
            cleaner: bookingData.cleanerId && bookingData.cleanerId > 0 ? bookingData.cleanerId : null,
            cleaner_rate: null as number | null,
            cleaning_type: bookingData.subServiceType === 'domestic' ? 'Domestic' : 'Commercial',
          ...
          if (recurringError) {
            ...
            console.log('[useAirbnbBookingSubmit] Recurring service created successfully with cost:', recurringCost);
          }
