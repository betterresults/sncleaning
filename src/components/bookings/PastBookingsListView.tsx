...
const PastBookingsListView = ({ dashboardDateFilter }: PastBookingsListViewProps) => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (dashboardDateFilter) {
        ...
        bookingsQuery = bookingsQuery.lt('date_time', new Date().toISOString());
      }
    ...
    return (
      <div className="flex flex-col items-center justify-center py-6 px-4">
        <div className="rounded-3xl bg-gradient-to-br from-gray-50 to-gray-100 p-6 shadow-[0_8px_30px_rgb(0,0,0,0.08)] text-center max-w-sm">
          ...
          <p className="text-xs text-gray-500">Select another period or adjust filters</p>
        </div>
      </div>
    );
  }
  ...
  // Show filters only when displaying all past bookings (no dashboardDateFilter)
  ...
  return (
    <div className="space-y-4">
      ...
      <ManualPaymentDialog
        booking={selectedBookingForPayment as any}
